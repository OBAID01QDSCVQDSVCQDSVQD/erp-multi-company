...
interface AvoirFournisseurData {
    numero: string;
    dateDoc: string;
    documentType: string;
    supplierName?: string;
    ...
    totalFodec?: number;
    timbreFiscal?: number;
    totalBaseHT?: number; // Total Brut before global discount
}

// ... existing code ...

function drawTotals(doc: jsPDF, data: AvoirFournisseurData, startY: number, maxHeightBeforeFooter: number): number {
    const x = 125;
    const w = 75;
    const lineH = 6;
    ...
    let lineCount = 0;
    const hasRemiseGlobale = (data.remiseGlobalePct || 0) > 0;
    const hasFodec = (data.totalFodec || 0) > 0;
    const hasTimbre = (data.timbreFiscal || 0) > 0;

    if (hasRemiseGlobale) {
        lineCount += 3; // Total Brut, Remise, Total Net
    } else {
        lineCount += 1; // Total HT
    }

    if (hasFodec) lineCount += 1;
    lineCount += 1; // Total TVA
    if (hasTimbre) lineCount += 1;
    // Total TTC is separate logic (footer of box)
    ...
    if (hasRemiseGlobale) {
        ...
        doc.text('Total HT Brut', x + 4, y);
        doc.setFont('helvetica', 'bold');
        doc.text(`${(data.totalBaseHT || data.totalHT).toFixed(3)}${currencySymbol}`, x + w - 4, y, { align: 'right' });
        y += lineH;

        // Remise Globale
        doc.setFont('helvetica', 'normal');
        doc.text(`Remise Globale (${data.remiseGlobalePct}%)`, x + 4, y);
        doc.text(`-${((data.totalBaseHT || data.totalHT) * (data.remiseGlobalePct! / 100)).toFixed(3)}${currencySymbol}`, x + w - 4, y, { align: 'right' });
        y += lineH;

        // Total HT Net
        doc.setFont('helvetica', 'bold');
        doc.text('Total HT Net', x + 4, y);
    ...
    if (hasFodec) {
        doc.setFont('helvetica', 'normal');
        doc.text('Fodec 1%', x + 4, y);
        doc.setFont('helvetica', 'bold');
        doc.text(`${data.totalFodec!.toFixed(3)}${currencySymbol}`, x + w - 4, y, { align: 'right' });
        y += lineH;
    }
    ...
    if (hasTimbre) {
        doc.setFont('helvetica', 'normal');
        doc.text('Timbre Fiscal', x + 4, y);
        doc.setFont('helvetica', 'bold');
        doc.text(`${data.timbreFiscal!.toFixed(3)}${currencySymbol}`, x + w - 4, y, { align: 'right' });
        y += lineH;
    }
...
function drawTitle(doc: jsPDF): number {
    ...
    doc.setTextColor(220, 38, 38); // Red color for Credit Notes/Returns
    doc.text('AVOIR FOURNISSEUR', 10, startY);
    doc.setTextColor(0, 0, 0);

    return startY + 8;
}

function drawInfoBlocks(doc: jsPDF, data: AvoirFournisseurData, companyInfo: CompanyInfo, startY: number): number {
    const col1X = 12;
    const col2X = 75;
    const supplierX = 120;
    const h = 32;
    ...
    doc.setFontSize(9).setFont('helvetica', 'normal');
    doc.text('Numéro d avoir', col1X, startY + 4);
    doc.setTextColor(220, 38, 38);
    doc.setFont('helvetica', 'bold');
    ...
    if (data.referenceExterne) {
        doc.text('Facture liée', col1X, startY + 17);
        doc.setFont('helvetica', 'bold');
        doc.text(data.referenceExterne, col1X, startY + 23);
        doc.setFont('helvetica', 'normal');
    }
    ...
    doc.setFont('helvetica', 'bold');
    const statutText = data.statut === 'VALIDE' || data.statut === 'VALIDEE' ? 'Validé' : data.statut === 'ANNULE' ? 'Annulé' : 'Brouillon';
...
function drawLinesTable(doc: jsPDF, data: AvoirFournisseurData, startY: number): number {
...
export function generateAvoirFournisseurPdf(data: AvoirFournisseurData, companyInfo: CompanyInfo): jsPDF {
