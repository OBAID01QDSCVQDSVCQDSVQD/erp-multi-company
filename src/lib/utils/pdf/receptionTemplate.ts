...
function drawTotals(doc: jsPDF, receptionData: ReceptionData, startY: number, maxHeightBeforeFooter: number): number {
  try {
    const x = 125;
    const w = 75;
    const lineH = 6;
    ...
    // Show remise globale if percentage is set
    ...
    // Return the new Y position after totals (could be used for something else)
...
export function generateReceptionPdf(receptionData: ReceptionData, companyInfo: CompanyInfo): jsPDF {
  ...
  let tableEndY = drawLinesTable(doc, receptionData, y);

  // Calculate space for totals
  let lineCount = 2; // Total HT + Total TVA
  // ... calculation from drawTotals ...
  // Assume generic height for check
  ...
  if (receptionData.notes) {
    ...
    // Simple check: if less than 30mm left, new page
    if (availableSpace < 30) {
      doc.addPage();
      tableEndY = 10;
      doc.setPage(doc.getNumberOfPages());
    }
    tableEndY = drawNotes(doc, receptionData.notes, tableEndY);
  }

  // Draw totals
  // Check space again for totals
