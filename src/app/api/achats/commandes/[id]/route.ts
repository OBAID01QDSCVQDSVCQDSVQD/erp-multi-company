...
export async function GET(
  ...
  try {
    ...
    const commande = await (PurchaseOrder as any).findOne({ 
      _id: id, 
      societeId: tenantId
    }).lean();
    ...
    console.error('Erreur GET /achats/commandes/[id]:', error);
...
export async function PATCH(
  ...
  try {
    ...
    const commande = await (PurchaseOrder as any).findOne({ _id: id, societeId: tenantId });
    if (!commande) {
      return NextResponse.json({ error: 'Commande non trouvée' }, { status: 404 });
    }

    // Update fields
    Object.assign(commande, body);
    ...
    console.error('Erreur PATCH /achats/commandes/[id]:', error);
    return NextResponse.json(
      { error: 'Erreur serveur', details: (error as Error).message },
      { status: 500 }
    );
...
export async function DELETE(
  ...
  try {
    ...
    await (commande as any).deleteOne();

    return NextResponse.json({ message: 'Commande supprimée' });
  } catch (error) {
    console.error('Erreur DELETE /achats/commandes/[id]:', error);
    return NextResponse.json(
      { error: 'Erreur serveur', details: (error as Error).message },
      { status: 500 }
    );
