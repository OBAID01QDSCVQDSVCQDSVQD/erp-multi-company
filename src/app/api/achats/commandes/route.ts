...
export async function GET(request: NextRequest) {
  try {
    ...
    const commandes = await (PurchaseOrder as any).find(query)
      .sort('-createdAt')
      .skip((page - 1) * limit)
      .limit(limit)
      .lean();
    ...
    return NextResponse.json({ items: commandes, total });
  } catch (error) {
    console.error('Erreur GET /achats/commandes:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const commande = new PurchaseOrder({
      ...body,
      societeId: tenantId,
      numero,
    ...
    // Calculate totals
    calculatePurchaseOrderTotals(commande);

    await (commande as any).save();

    return NextResponse.json(commande, { status: 201 });
  } catch (error) {
    console.error('Erreur POST /achats/commandes:', error);
    return NextResponse.json(
      { error: 'Erreur serveur', details: (error as Error).message },
      { status: 500 }
    );
...
// Helper function to calculate purchase order totals
function calculatePurchaseOrderTotals(po: any) {
  ...
  // Add timbre fiscal if it exists
  const timbreFiscal = po.timbreFiscal || 0;
  po.totalTTC = po.totalBaseHT + po.totalTVA + timbreFiscal;

  return { totalBaseHT, totalRemise, totalTVA, totalTTC: po.totalTTC };
}
