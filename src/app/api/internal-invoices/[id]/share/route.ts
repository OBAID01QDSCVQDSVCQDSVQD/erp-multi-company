...
export async function POST(
    ...
    try {
        ...
        // Check if token exists AND starts with INT-
        if (doc.publicToken && doc.publicToken.startsWith('INT-')) {
            return NextResponse.json({ token: doc.publicToken });
        }

        // Generate descriptive token: INT-[Numero]-[Random]
        const randomSuffix = crypto.randomBytes(3).toString('hex');
        const sanitizedNum = doc.numero.replace(/[^a-zA-Z0-9-_]/g, '-');
        const token = `INT-${sanitizedNum}-${randomSuffix}`;
