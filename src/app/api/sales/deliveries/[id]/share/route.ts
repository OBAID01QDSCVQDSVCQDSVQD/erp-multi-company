...
export async function POST(
    ...
    try {
        ...
        // Check if token exists AND starts with BL-
        if (doc.publicToken && doc.publicToken.startsWith('BL-')) {
            return NextResponse.json({ token: doc.publicToken });
        }

        // Generate descriptive token: BL-[Numero]-[Random]
        const randomSuffix = crypto.randomBytes(3).toString('hex');
        const sanitizedNum = doc.numero.replace(/[^a-zA-Z0-9-_]/g, '-');
        const token = `BL-${sanitizedNum}-${randomSuffix}`;
