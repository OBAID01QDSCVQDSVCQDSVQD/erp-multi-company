...
import { PlusIcon, TruckIcon, MagnifyingGlassIcon, EyeIcon, PencilIcon, ArrowDownTrayIcon, TrashIcon, DocumentTextIcon, XMarkIcon, ArrowLeftIcon } from '@heroicons/react/24/outline';
...
export default function PurchaseOrdersPage() {
  ...
  const handleSelectSupplier = (supplier: Supplier) => {
    setFormData({ ...formData, fournisseurId: supplier._id });
    setSupplierSearch(supplier.raisonSociale || `${supplier.nom || ''} ${supplier.prenom || ''}`.trim());
    setShowSupplierDropdown(false);
  ...
  useEffect(() => {
    if (showModal && tenantId) {
      setLoadingData(true);
      Promise.all([fetchSuppliers(), fetchProducts(), fetchTaxRates(), fetchTvaSettings(), fetchModesReglement()]).finally(() => {
        ...
        const ordersWithSuppliers = await Promise.all(
          orders.map(async (order: PurchaseOrder) => {
            if (order.fournisseurId) {
              try {
                const supplierResponse = await fetch(`/api/suppliers/${order.fournisseurId}`, {
                  headers: { 'X-Tenant-Id': tenantId }
                });
  ...
  const filtered = orders.filter(order =>
    ...
    (order.fournisseurNom && order.fournisseurNom.toLowerCase().includes(q.toLowerCase()))
  );
  ...
  const handleView = async (order: PurchaseOrder) => {
    try {
      ...
      if (response.ok) {
        ...
        toast.success(`Commande ${order.numero} chargée avec succès`);
        router.push(`/purchases/orders/${order._id}`);
      } else {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          dateDoc: fullOrder.dateDoc?.split('T')[0] || new Date().toISOString().split('T')[0],
          adresseLivraison: fullOrder.adresseLivraison || '',
          devise: fullOrder.devise || 'TND',
          conditionsPaiement: fullOrder.conditionsPaiement || '',
        ...
        if (fullOrder.fournisseurId) {
          ...
          if (selectedSupplier) {
            setSupplierSearch(selectedSupplier.raisonSociale || `${selectedSupplier.nom || ''} ${selectedSupplier.prenom || ''}`.trim());
          }
        } else {
          setSupplierSearch('');
        }
  ...
  const handleDelete = async (orderId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success('Commande d\'achat supprimée avec succès');
        fetchOrders();
      } else {
        const error = await response.json();
        toast.error(error.error || 'Erreur lors de la suppression de la commande');
      }
    } catch (err) {
                            ...
                            <div className="overflow-y-auto max-h-[240px]">
                              {filteredSuppliers.length > 0 ? (
                                filteredSuppliers.map((supplier, index) => {
                                  const displayName = supplier.raisonSociale || `${supplier.nom || ''} ${supplier.prenom || ''}`.trim();
                                  const secondaryInfo = [
