...
interface Transaction {
    id: string;
    type: 'facture' | 'paiement' | 'avoir' | 'devis' | 'commande' | 'livraison';
    numero: string;
    reference: string;
    date: string;
    ...
    modePaiement?: string;
    documentType: string;
    invoiceType?: string; // 'FAC', 'INT_FAC', 'DEVIS', 'BC', 'BL'
    lignes?: Array<{
...
export default function CustomerDetailsPage() {
    ...
    const getTransactionTypeIcon = (type: string) => {
        switch (type) {
            ...
            case 'devis': return <ClipboardDocumentListIcon className="w-5 h-5 text-gray-600" />;
            case 'commande': return <ShoppingCartIcon className="w-5 h-5 text-indigo-600" />;
            case 'livraison': return <TruckIcon className="w-5 h-5 text-teal-600" />;
            default: return null;
        }
    };
    ...
    const handleExport = async () => {
        ...
        // Reuse existing logic but ensuring it passes filters
    };

    const handleViewDocument = (transaction: Transaction) => {
        if (transaction.type === 'devis') {
            router.push(`/sales/quotes/${transaction.id}`);
        } else if (transaction.type === 'commande') {
            router.push(`/sales/orders/${transaction.id}`);
        } else if (transaction.type === 'livraison') {
            router.push(`/sales/deliveries/${transaction.id}`);
        } else if (transaction.documentType === 'Document') {
            ...
            <div className="p-6 space-y-6">
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Détails Client</h1>
                            {customer && (
                                <div className="flex flex-col">
                                    <span className="text-lg font-medium text-gray-900 dark:text-white">{customer.nom}</span>
                                    <span className="text-sm text-gray-500">{customer.email} {customer.telephone ? `• ${customer.telephone}` : ''}</span>
                                </div>
                            )}
                        </div>
                    </div>
                ...
                <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border dark:border-gray-700 space-y-4">
                    ...
                    <div className="border-b border-gray-200 dark:border-gray-700 overflow-x-auto">
                        <nav className="-mb-px flex space-x-6" aria-label="Tabs">
                            {['facture', 'paiement', 'devis', 'commande', 'livraison'].map((type) => (
                                <button
                                    key={type}
                                ...
                                >
                                    ...
                                    {type === 'facture' ? 'Factures' : type === 'paiement' ? 'Paiements' : type === 'devis' ? 'Devis' : type === 'commande' ? 'Commandes' : 'Livraisons'}
                                </button>
                            ))}
                        </nav>
                    </div>
