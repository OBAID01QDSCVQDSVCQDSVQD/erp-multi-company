...
export default function NewExpensePage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (taxRatesRes.ok) {
        ...
        console.error('Erreur lors du chargement des taux TVA');
      }
    } catch (err) {
  ...
  // Gérer la sélection de "Créer une catégorie"
  useEffect(() => {
    if (watchedCategorieId === 'create') {
      handleCreateCategory();
      setValue('categorieId', ''); // Réinitialiser la sélection
    }
  ...
  const onSubmit = async (data: ExpenseForm) => {
    ...
    try {
          // Trouver le taux de TVA sélectionné
          const selectedTaxRate = taxRates.find(rate => rate.code === data.taxCode);
          
          // Convertir les images en structure piecesJointes
