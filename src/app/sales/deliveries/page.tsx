...
export default function DeliveriesPage() {
  ...
  const handleOpenWhatsAppModal = async (delivery: Delivery) => {
    ...
    // The interface says string | object.

    let custId = '';
    if (delivery.customerId) {
      if (typeof delivery.customerId === 'object') {
        custId = (delivery.customerId as any)._id;
      } else {
        custId = delivery.customerId as string;
      }
    }
  ...
  const confirmWhatsAppSend = async () => {
    if (!selectedDeliveryForWhatsApp || !whatsAppNumber) return;
    ...
    let publicLink = '';
    const delivery = selectedDeliveryForWhatsApp;
    ...
    const customerName = delivery.customerName || 'Cher Client';
                        ...
                        <td className="px-2 py-4">
                          <div className="flex gap-0.5">
                            ...
                            <button
                              ...
                              disabled={downloadingId === delivery._id}
                            ...
                            <button
                              onClick={() => {
                                handleOpenWhatsAppModal(delivery);
                              }}
              ...
              {filtered.map((delivery) => (
                <div key={delivery._id} className="bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-xl shadow-sm p-4 space-y-3">
                  ...
                  <div className="flex gap-2 pt-2">
                    ...
                    <button
                      onClick={() => handleOpenWhatsAppModal(delivery)}
