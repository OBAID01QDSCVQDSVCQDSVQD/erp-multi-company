      ...
      <div className="p-6 space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border p-6">
          <div className="grid grid-cols-2 gap-6 mb-6">
            <div>
              <label className="text-sm text-gray-600">Numéro de devis</label>
              <p className="text-lg font-bold text-blue-600">{quote.numero}</p>
            </div>
            <div>
              <label className="text-sm text-gray-600">Date</label>
              <p className="text-lg font-medium">{new Date(quote.dateDoc).toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit', year: 'numeric' })}</p>
            </div>
            <div>
                  ...
                  {quote.lignes.map((line: any, index: number) => {
                    ...
                    return (
                      <tr key={index} className={index % 2 === 0 ? 'bg-blue-50' : 'bg-pink-50'}>
                        ...
                        <td className="px-4 py-3 text-sm">{line.prixUnitaireHT?.toFixed(3)} {quote.devise}</td>
            ...
            <div className="w-80 bg-blue-50 border border-blue-100 rounded-lg p-4 space-y-3">
              ...
              {quote.timbreFiscal && quote.timbreFiscal > 0 && (
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-gray-900">{quote.timbreFiscal.toFixed(3)} {quote.devise}</span>
                </div>
              )}
              <div className="border-t border-blue-200 pt-3 flex justify-between text-lg font-bold">
                <span className="text-gray-900">Total TTC</span>
                <span className="text-blue-600">{quote.totalTTC?.toFixed(3)} {quote.devise}</span>
              </div>
            </div>
          </div>
