...
import ProjectSearchModal from '@/components/common/ProjectSearchModal';
                          ...
                          {record ? (
                            projects.length > 0 ? (
                              <ProjectSelector
                                ...
                                disabled={loading}
                                onChange={(projectId) => handleProjectChange(record._id, projectId)}
                                title={`Associer un projet à ${employee.firstName} ${employee.lastName}`}
                              />
                            ) : record.projectId ? (
...
function ProjectSelector({ currentProjectId, projects, disabled, onChange, title }: ProjectSelectorProps) {
  ...
  return (
    <>
      <button
        ...
        className={`w-full sm:w-56 px-3 py-2.5 border rounded-xl text-sm font-medium flex items-center justify-between transition-all duration-200 group ${disabled
          ? 'bg-gray-100 dark:bg-gray-800 text-gray-400 dark:text-gray-500 cursor-not-allowed border-gray-200 dark:border-gray-700'
          : selectedProject
            ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-400 text-blue-700 dark:text-blue-300 hover:bg-blue-100 dark:hover:bg-blue-900/30'
            : 'bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 hover:border-blue-400 dark:hover:border-blue-500 hover:shadow-sm'
          }`}
      >
        <div className="flex items-center gap-2 truncate">
          <BriefcaseIcon className={`w-4 h-4 flex-shrink-0 ${selectedProject ? 'text-blue-600' : 'text-gray-400'}`} />
          <span className="truncate">
            {selectedProject ? selectedProject.name : 'Associer à un projet'}
          </span>
        </div>
        <svg
          className={`w-4 h-4 ml-2 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''} ${selectedProject ? 'text-blue-500' : 'text-gray-400'}`}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
