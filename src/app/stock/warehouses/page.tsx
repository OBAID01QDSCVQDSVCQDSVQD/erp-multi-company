            ...
            <div className="p-6 max-w-7xl mx-auto space-y-6">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        ...
                        <p className="text-gray-500 mt-1">Gérez vos dépôts, showrooms et véhicules de livraison.</p>
                    </div>
                    <button
                        onClick={() => openModal()}
                ...
                {/* Content */}
                {loading ? (
                    <div className="text-center py-12">Checking warehouses...</div>
                ) : warehouses.length === 0 ? (
                    <div className="text-center py-20 bg-gray-50 rounded-xl border border-dashed border-gray-300">
                        <BuildingOfficeIcon className="w-16 h-16 text-gray-300 mx-auto mb-4" />
                        <h3 className="text-lg font-medium text-gray-900">Aucun entrepôt défini</h3>
                        <p className="text-gray-500 mb-6">Commencez par créer votre entrepôt principal.</p>
                        <button onClick={() => openModal()} className="text-blue-600 font-medium hover:underline">Créer maintenant</button>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {warehouses.map((wh) => (
                            <div key={wh._id} className={`bg-white rounded-xl shadow-sm border p-5 relative group transition hover:shadow-md ${wh.isDefault ? 'border-blue-200 ring-1 ring-blue-100' : 'border-gray-200'}`}>
                                {wh.isDefault && (
                                ...
                                <div className="flex items-start gap-4 mb-4">
                                    ...
                                    <div>
                                        ...
                                        <span className="inline-block px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-600 mt-1">
                                            {typeLabels[wh.type]}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="border-t pt-4 flex justify-end gap-2 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button
                        ...
                        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select {...register('type')} className="w-full rounded-lg border-gray-300">
                                ...
                                <div>
                                    ...
                                    <input {...register('address.city')} type="text" className="w-full rounded-lg border-gray-300" placeholder="Tunis..." />
                                </div>
                            </div>

                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Adresse complète</label>
                                <input {...register('address.street')} type="text" className="w-full rounded-lg border-gray-300" placeholder="Rue..." />
                            </div>

                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Responsable</label>
                                <input {...register('manager.name')} type="text" className="w-full rounded-lg border-gray-300" placeholder="Nom du responsable" />
                            </div>
                            ...
                            <div className="flex items-center gap-2 pt-2">
                                ...
                                />
                                <label htmlFor="isDefault" className="text-sm text-gray-700">Définir comme entrepôt par défaut</label>
                            </div>
